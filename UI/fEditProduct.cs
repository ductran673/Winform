...
namespace UI
{
    public partial class fEditProduct : Form
    {
        ...
        private void btSave_Click(object sender, EventArgs e)
        {
            ...
            {
                toolTip1.Show("Hãy nhập địa chỉ khách hàng?", mPrice, 0, 0, 1000);
                mPrice.Focus();
                return;
            }
            ...
            try
            {
                ...
                Product.Price = Convert.ToInt32(mPrice.Text);
        ...
        private void mPrice_Validating(object sender, System.ComponentModel.CancelEventArgs e)
        {
            if (string.IsNullOrWhiteSpace(mPrice.Text))
            {
                toolTip1.Show("Hãy nhập địa chỉ?", mPrice, 0, 0, 1000);
                e.Cancel = true;
            }
            else if (mPrice.Text.Length > 250)
            {
                toolTip1.Show("Địa chỉ <= 250 ký tự?", mPrice, 0, 0, 1000);
                e.Cancel = true;
            }
        }
